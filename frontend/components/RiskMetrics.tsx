...
export const RiskMetrics = memo(function RiskMetrics({ risk, portfolio }: { risk: RiskMetricsData | null; portfolio: PortfolioData | null }) {
  const metrics = [
    { label: "VAR 95%", value: `$${(risk?.var_95 ?? 250).toFixed(0)}`, pct: Math.min(((risk?.var_95 ?? 250) / 1000) * 100, 100), r: Math.min((risk?.var_95 ?? 250) / 1000, 1) },
  ...
  return (
    <div className="bg-bb-surface border border-bb-border h-full flex flex-col overflow-hidden">
      ...
      <div className="px-2 py-1.5 space-y-1.5 flex-1 min-h-0 overflow-y-auto">
        {metrics.map((m) => (
          <div key={m.label}>
