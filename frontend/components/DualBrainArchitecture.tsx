      ...
      <div className="grid-cell-header">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="bg-bb-black border border-bb-border px-2 py-1 text-[10px] text-bb-amber max-w-[140px]"
          >
            {symbols.map((s) => (
              <option key={s} value={s}>{s}</option>
            ))}
          </select>
          <span className={`text-[10px] uppercase font-bold ${data?.mode === "live" ? "text-bb-green" : "text-bb-amber"}`}>
            {data?.mode || "loading"}
          </span>
        </div>
      </div>

      <div className="grid-cell-body overflow-auto p-3 lg:p-4">
        {/* Two-column brain layout - stacks on mobile */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
          <TABrainPanel ta={ctx?.ta} regime={ctx?.regime} />
          <FundBrainPanel fund={ctx?.fund} />
        </div>

        {/* Lower panels - 3 column on desktop, stacked on mobile */}
...
function TABrainPanel({ ta, regime }: { ta?: TABrainOutput; regime?: string }) {
  ...
  return (
    <div className="border border-bb-border bg-bb-panel p-3 lg:p-4">
      <div className="text-[11px] font-bold text-bb-orange mb-3 tracking-wider">
        TA BRAIN <span className="text-bb-dim font-normal">(Nunchi 14 Strats)</span>
      </div>

      {/* APEX Orchestrator */}
      <div className="border border-bb-border bg-bb-surface p-3 mb-3">
        <div className="text-[10px] text-bb-cyan font-bold mb-2">APEX ORCHESTRATOR</div>
        <div className="grid grid-cols-3 gap-3 text-[9px] text-bb-dim">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="border border-bb-border bg-bb-black p-3 text-center">
          <div className="text-[8px] text-bb-dim mb-1.5 tracking-wider">RADAR (15m)</div>
          <div className={`text-[20px] font-bold leading-none ${getRadarColor(radarScore)}`}>{radarScore}</div>
          <div className="text-[8px] text-bb-dim mt-1.5">{radarTier}</div>
        </div>
        <div className="border border-bb-border bg-bb-black p-3 text-center">
          <div className="text-[8px] text-bb-dim mb-1.5 tracking-wider">PULSE (60s)</div>
          <div className="text-[20px] font-bold text-bb-amber leading-none">T{pulseTier}</div>
          <div className="text-[8px] text-bb-dim mt-1.5">{pulseDir}</div>
        </div>
        <div className="border border-bb-border bg-bb-black p-3 text-center">
          <div className="text-[8px] text-bb-dim mb-1.5 tracking-wider">GUARD (tick)</div>
          <div className="text-[20px] font-bold text-bb-cyan leading-none">2&#966;</div>
          <div className="text-[8px] text-bb-dim mt-1.5">stops</div>
        </div>
      </div>

      {/* 14 Strategies + Signals side by side */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
        <div className="border border-bb-border bg-bb-surface p-3">
          <div className="text-[10px] text-bb-cyan font-bold mb-2">14 STRATEGIES</div>
          <div className="text-[9px] text-bb-dim space-y-1.5">
        ...
        <div className="border border-bb-border bg-bb-surface p-3">
          <div className="text-[10px] text-bb-cyan font-bold mb-2">LIVE SIGNALS</div>
          <div className="space-y-2">
            {strategies.slice(0, 5).map((s, i) => (
              <div key={i} className="flex items-center justify-between text-[9px] border-b border-bb-border/50 pb-1.5">
...
function FundBrainPanel({ fund }: { fund?: FundBrainOutput }) {
  ...
  return (
    <div className="border border-bb-border bg-bb-panel p-3 lg:p-4">
      <div className="text-[11px] font-bold text-bb-orange mb-3 tracking-wider">
        FUNDAMENTAL BRAIN <span className="text-bb-dim font-normal">(Multi-Src)</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        {/* Whale Tracker */}
        <div className="border border-bb-border bg-bb-surface p-3">
          <div className="text-[10px] text-bb-cyan font-bold mb-2">WHALE TRACKER</div>
          <div className="text-[9px] text-bb-dim mb-1.5">OnchainOS smart money</div>
          <div className="flex justify-between items-center">
            <span className="text-[9px] text-bb-dim">Sentiment</span>
            <span className={`text-[13px] font-bold ${getSentimentColor(whaleSent)}`}>
        ...
        {/* Twitter/X Sentiment */}
        <div className="border border-bb-border bg-bb-surface p-3">
          <div className="text-[10px] text-bb-cyan font-bold mb-2">TWITTER/X</div>
          <div className="text-[9px] text-bb-dim mb-1.5">KOL mentions{twitterData ? ` (${twitterData.mention_count})` : ""}</div>
          <div className="flex justify-between items-center">
            <span className="text-[9px] text-bb-dim">Sentiment</span>
            <span className={`text-[13px] font-bold ${getSentimentColor(twitterSent)}`}>
        ...
        {/* News RSS Feeds */}
        <div className="border border-bb-border bg-bb-surface p-3">
          <div className="text-[10px] text-bb-cyan font-bold mb-2">NEWS RSS</div>
          <div className="text-[9px] text-bb-dim mb-1.5">CoinDesk, The Block, Decrypt</div>
          <div className="flex justify-between items-center">
            <span className="text-[9px] text-bb-dim">Sentiment</span>
            <span className={`text-[13px] font-bold ${getSentimentColor(newsSent)}`}>
        ...
        {/* Security Scanner */}
        <div className="border border-bb-border bg-bb-surface p-3">
          <div className="text-[10px] text-bb-cyan font-bold mb-2">SECURITY SCAN</div>
          <div className="text-[9px] text-bb-dim space-y-1.5">
      ...
      {twitterData && twitterData.kol_mentions.length > 0 && (
        <div className="border border-bb-border bg-bb-surface p-3 mb-3">
          <div className="text-[10px] text-bb-cyan font-bold mb-2">KOL FEED</div>
  ...
  return (
    <div className="border border-bb-border bg-bb-surface p-3 h-full">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`text-[8px] px-2 py-0.5 font-bold ${isConflict ? "bg-bb-red/20 text-bb-red border border-bb-red/50" : "bg-bb-green/20 text-bb-green border border-bb-green/50"}`}>
      ...
      <div className="text-[8px] text-bb-muted mt-3 space-y-1">
  ...
  return (
    <div className="border border-bb-border bg-bb-panel p-3 h-full">
      <div className="text-[10px] font-bold text-bb-orange tracking-wider mb-3">
        CLAUDE ORCHESTRATOR <span className="text-bb-dim font-normal">(/simmons-dual)</span>
      </div>

      <div className="space-y-2">
        <div className="border border-bb-border bg-bb-black p-2.5">
          <div className="text-[9px] text-bb-cyan font-bold mb-1">Multi-Agent Debate</div>
          <div className="text-[10px] text-bb-dim">Bull / Bear / Risk</div>
        </div>
        <div className="border border-bb-border bg-bb-black p-2.5">
          <div className="text-[9px] text-bb-cyan font-bold mb-1">Strategy Select</div>
          <div className="text-[10px] text-bb-dim">MM vs ARB vs DIR</div>
        </div>
        <div className="border border-bb-border bg-bb-black p-2.5">
          <div className="text-[9px] text-bb-cyan font-bold mb-1">Position Sizing</div>
          <div className="text-[10px] text-bb-dim">Kelly: {((parseFloat(ctx?.size_factor ?? "0") * 100)).toFixed(0)}%</div>
        </div>
        <div className="border border-bb-border bg-bb-black p-2.5">
          <div className="text-[9px] text-bb-cyan font-bold mb-1">GUARD Stops</div>
          <div className="text-[10px] text-bb-dim">Synced</div>
        </div>
      </div>
    </div>
  );
...
function ExecutionPanel({ mode }: { mode?: string }) {
  return (
    <div className="border border-bb-border bg-bb-surface p-3 h-full">
      ...
      <div className="space-y-2">
        <div className={`border p-2.5 ${mode === "paper" ? "border-bb-amber bg-bb-amber/10" : "border-bb-border bg-bb-black"}`}>
          <div className="text-[9px] text-bb-amber font-bold mb-1">Paper</div>
          <div className="text-[10px] text-bb-dim">Simmons Rust engine</div>
        </div>
        <div className={`border p-2.5 ${mode === "live" ? "border-bb-green bg-bb-green/10" : "border-bb-border bg-bb-black"}`}>
          <div className="text-[9px] text-bb-green font-bold mb-1">Live DEX</div>
          <div className="text-[10px] text-bb-dim">OnchainOS swap</div>
        </div>
        <div className="border border-bb-border bg-bb-black p-2.5">
          <div className="text-[9px] text-bb-blue font-bold mb-1">Live Perps</div>
          <div className="text-[10px] text-bb-dim">Hyperliquid (opt)</div>
        </div>
      </div>

      <div className="text-[9px] text-bb-muted mt-3">
